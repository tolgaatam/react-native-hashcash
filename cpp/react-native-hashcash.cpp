...
namespace
{
  ...
  inline uint32_t f2(uint32_t a, uint32_t b, uint32_t c)
  {
	...
	std::string calculateHashcash(int k, std::string identifier){
		...
		oss << std::setfill('0') << std::setw(2) << (pTInfo->tm_year + 1900) % 100;
		oss << std::setfill('0') << std::setw(2) << pTInfo->tm_mon+1;
		oss << std::setfill('0') << std::setw(2) << pTInfo->tm_mday;

		oss << ":" << identifier;
		oss << ":" << extraParams;
		oss << ":" << gen_random(saltLength);
		oss << ":";

		std::string baseString = oss.str();

		// MULTITHREADED VERSION
		unsigned short processor_count = std::thread::hardware_concurrency();
		if(processor_count == 0) // not detected.. assume 4.
			processor_count = 4;
		...
		unsigned short thread_count_log = int_log(thread_count);

		std::atomic<long> lastCounterEnd{0};
		long iterationPerThreadPerRound = 1 << (long(k) - 3 - long(thread_count_log));

		std::vector<std::thread> threads;
		std::vector<std::string> results(thread_count, "");
		for(unsigned short i = 0; i < thread_count; i++){
			threads.push_back(std::thread(solve_thread, k, baseString, &results[i], iterationPerThreadPerRound, &lastCounterEnd));
		}
		...
		return ""; // no way that we reach this.
	}
}
...
std::string gen_random(short len){
	...
	for (short i = 0; i < len; ++i) {
		char randomChar = alphabet[ rand()%(alphabet.length()) ];
		s += randomChar;
	}
    return s;
 }

void into_hex(char* str, int num){
	snprintf(str, 20, "%x", num);
 }
...
int int_log(int num){
 ...
 void solve_thread(int k, std::string baseString, std::string* result, long iterationPerThreadPerRound, std::atomic<long> * lastCounterEnd){
	while(true){
		long counterStart = lastCounterEnd->fetch_add(iterationPerThreadPerRound, std::memory_order_relaxed);
		if(counterStart < 0){
			return;
		}
		long counterEnd = counterStart+ iterationPerThreadPerRound;

		//std::cout << counterStart << " " << counterEnd << std::endl;

		int hexDigitsToConsider;
		if(k % 4 == 0){
			hexDigitsToConsider = k / 4;
		} else {
			hexDigitsToConsider = k / 4 + 1;
		}

		int limitBeginningSetBits = hexDigitsToConsider * 4 - k;
		int limitBeginningNum = 1;

		for(int i = 0; i < limitBeginningSetBits; i++){
			limitBeginningNum *= 2;
		}

		std::ostringstream oss;
		oss << std::setfill('0') << std::setw(hexDigitsToConsider) << limitBeginningNum;
		std::string limitBeginning = oss.str();

    	hashcash::SHA256 sha256;

		for(long counter = counterStart; counter < counterEnd; counter++){
			std::string hexDigest = sha256(baseString + std::to_string(counter));

			if(hexDigest < limitBeginning){
				*lastCounterEnd = (long(1) << (47)) * -1;
				*result = baseString + std::to_string(counter);
				return;
			}
		}
	}
